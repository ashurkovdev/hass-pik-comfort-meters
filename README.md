...
- Отправка новых показаний через сервис (вручную или автоматически)
- Немедленное обновление данных после отправки показаний
...
- Изменение пароля и настроек без перенастройки интеграции
...
После добавления интеграции можно изменить настройки через меню интеграции:
1. Откройте интеграцию "PIK Comfort Meters"
2. Нажмите "Настроить" (Options)
3. Измените необходимые параметры:
   - **Интервал обновления** — интервал обновления данных в секундах (по умолчанию 21600 = 6 часов, диапазон: 3600-86400)
   - **Пароль** — новый пароль от личного кабинета (оставьте пустым, чтобы не менять)
4. Сохраните

> **Важно:** При изменении пароля интеграция автоматически переаутентифицируется с новыми учётными данными и обновит токен доступа.
...
Отправляет показания счётчика. Если показания не указаны, используются текущие значения сенсоров.

| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `device_id` | string | да | Идентификатор устройства из Device Registry |
| `readings` | number/list | нет | Показания для отправки. Может быть одним числом (для однотарифных счетчиков) или списком чисел (для многотарифных счетчиков, в порядке: тариф 1, тариф 2, тариф 3). Если не указано, используются текущие значения сенсоров. |

#### Пример вызова

Отправка текущих показаний сенсоров:
...
Отправка конкретных показаний (однотарифный счетчик):
...
Отправка конкретных показаний (двухтарифный счетчик):

```yaml
service: pik_comfort_meters.submit_reading
data:
  device_id: "1234567890abcdef"
  readings: [100.50, 50.25]  # [день, ночь]
```

Отправка конкретных показаний (трёхтарифный счетчик):

```yaml
service: pik_comfort_meters.submit_reading
data:
  device_id: "1234567890abcdef"
  readings: [100.50, 50.25, 30.10]  # [день, ночь, пик]
```

Или через UI: Developer Tools → Services → выберите сервис → выберите устройство в поле "Device" и при необходимости укажите показания.
...
action:
  - service: pik_comfort_meters.submit_reading
    data:
      device_id: "1234567890abcdef"  # замените на реальный device_id
      readings: [100.50, 50.25, 30.10]  # замените на показания со считывающих устройств
mode: single
```
